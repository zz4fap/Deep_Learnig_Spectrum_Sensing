...
for sample_length in [64, 128, 256, 512, 1024 ]:
    for mod_name in ['BPSK', 'QPSK', '8PSK', 'PAM4', 'QAM16', 'QAM64', 'GFSK', 'CPFSK']:
        ...
        best_model_path = 'result/models/LSTM/'+str(sample_length)+'/'+str(mod_name)+'best.h5'
        ...
        if model.stop_training:
            # save results
            model.save('result/models/LSTM/'+str(sample_length)+'/'+str(mod_name)+'final.h5')
            print('Second stage finished, get the final model')
            save_path = 'result/xls/LSTM/'+str(sample_length)+'/'+str(mod_name)+'Pds.xls'
