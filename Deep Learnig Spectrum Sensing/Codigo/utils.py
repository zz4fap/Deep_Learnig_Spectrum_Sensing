...
def radioml_IQ_data(filename, mod_name, swap_dim = False):
    '''
...
def radioml_IQ_CO_data(filename, mod_name):
    '''
...
def DetectNet(lr,input_shape,filter_num,lstm_units,kernel_size,drop_ratio,lstm_drop_ratio,dense_units):
    ...
    x1 = Convolution1D(filter_num,kernel_size,padding='same',data_format='channels_last',activation='relu')(ConvInput)
    x1 = Dropout(rate=drop_ratio)(x1)
    x2 = Convolution1D(filter_num,kernel_size,padding='same',data_format='channels_last',activation='relu')(x1)
