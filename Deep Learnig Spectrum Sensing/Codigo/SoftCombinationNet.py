...
#%% load data
dataset,labelset,SNR = utils.radioml_IQ_CO_data('pkl_data/coperative/3_'+str(sample_length)+'_co.pkl', mod_name)
...
#%% load single model
model_single = load_model('result/models/DetectNet/'+str(sample_length)+'/'+str(mod_name)+'final.h5')
flatten_dataset = np.reshape(dataset,(total_num,2,sample_length))

if(swap_dim):
    flatten_dataset = np.einsum('ijk->ikj', flatten_dataset)
...
#%%
signal_decisions = np.reshape(decisions[:total_num//2],(snr_type,total_num//2//snr_type)) #average snr
...
pd_list = np.append(pd_list,pf*100)
with open('result/xls/SoftCombinationNet/'+str(sample_length)+'/'+str(mod_name)+"_"+str(nodes) +'Pds.xls','w') as f:
    f_csv = csv.writer(f)
...
pd_hard_list = np.append(pd_hard_list,pf_hard*100)
with open('result/xls/SoftCombinationNet/'+str(sample_length)+'/'+str(mod_name)+"_"+str(nodes) +'Pds_hard.xls','w') as f:
    f_csv = csv.writer(f)
